...
export async function renderLectures(contentJson){
  ...
  const language = contentJson.category;
    ...
    for(const item of content){
      switch(item.type){
        ...
        case "list":
          ...
          break;
        case "code":
          const codeElement = await codeBlock(item.data,language);
          lectureElement.appendChild(codeElement);
        break;
        default:
          lectureElement.appendChild(el('p',{style: "color: red;"}, `${item.type} is not valid type bozo!!`));
          break;
      }
    }
